...
/***********************************Tin******************************************/
	...
	function Tin_add(&$loi){
		...
		if($thanhcong==true){
			...
			$sql="Insert into tin set lang='$lang',TieuDe='$TieuDe',TieuDe_KhongDau='$TieuDe_KhongDau',TomTat='$TomTat',urlHinh='$urlHinh',Ngay=now(),Content='$noidung',idUser={$_SESSION['login_id']},idLT=$idLT,TinNoiBat=$NoiBat,AnHien=$AnHien,idSK=0";
			$kq=$this->db->query($sql) or die ($this->db->error());
		}
		return $thanhcong;
	}
